...
_simpl_fifoPath = getenv("FIFO_PATH");
if (_simpl_fifoPath == NULL)
	{
...
if (_simpl_statFifoName(_simpl_myStuff.whom) != -1)
	{
	_simpl_setErrorCode(NAME_NOT_AVAILABLE);
	return(-1);
	}

// create a receive fifo (necessary for a receiver)
...
if (mkfifo(name, 0666) == -1)
	...
	_simpl_log("%s: unable to create send fifo %s-%s\n", fn, name, strerror(errno));
	return(-1);
	}
_simpl_myStuff.fd = -1; // we don't set fd until needed

// create a reply fifo (necessary for a sender)
...
if (mkfifo(name, 0666) == -1)
	{
	_simpl_setErrorCode(FIFO_CREATION_FAILURE);
	_simpl_log("%s: unable to create reply fifo %s-%s\n", fn, name, strerror(errno));
	return(-1);
	}
_simpl_myStuff.y_fd = -1; // we don't set y_fd until needed
...
if ( strlen(hostName) == 0 )
	...
	if ( !inet_aton(hostName, &remote) )
		...
		for (i = 0; local->h_addr_list[i]; i++)
			...
			if ( !strcmp(hostName, inet_ntoa( *(struct in_addr *)local->h_addr_list[i])) )
				{
...
// open the receive fifo (first time) for the sender to kick message 
if (_simpl_myStuff.fd == -1)
	{
	char fifoName[128];
...
if (*sender == (char *)-1)
	...
	_simpl_log("%s: cannot attach to shmem-%s\n", fn, strerror(errno));
	ReplyError(*sender);
	return(-1);
	}
